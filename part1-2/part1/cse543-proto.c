...
printf("break 0");
	...
	// buffer= (char*)malloc(MAX_BLOCK_SIZE); // CHECCCKKKK to include or not include
...
printf("break 3");
	...
	memcpy(buffer+sizeof(int) , &ivl, sizeof(int));
	memcpy(buffer+sizeof(int)+sizeof(int) ,&ciphertext_size, sizeof(int));
...
int unseal_symmetric_key( char *buffer, unsigned int len, EVP_PKEY *privkey, unsigned char **key )
{
	...
	*/
	...
	// char * temp= "";
	char * temp= malloc(MAX_BLOCK_SIZE);
...
int client_authenticate( int sock, unsigned char **session_key )
{
	...
	*/
	...
	char* encrypted_symmetric_key= (char*)malloc(MAX_BLOCK_SIZE);
